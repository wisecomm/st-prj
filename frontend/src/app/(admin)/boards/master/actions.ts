...
export async function getBoards(page: number, size: number, brdNm?: string, startDate?: string, endDate?: string): Promise<ApiResponse<PageResponse<BoardMaster>>> {
    try {
        ...
        const response = await apiClient.get<PageResponse<BoardMaster>>(`/v1/mgmt/boards/master?${params.toString()}`);
        return response;
    } catch (error: unknown) {
...
export async function getBoardById(brdId: string): Promise<ApiResponse<BoardMaster>> {
    try {
        const response = await apiClient.get<BoardMaster>(`/v1/mgmt/boards/master/${brdId}`);
        return response;
    } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
            return error.response?.data || { code: "500", message: error.message, data: null };
...
export async function createBoard(data: Partial<BoardMaster>): Promise<ApiResponse<void>> {
    try {
        const response = await apiClient.post<void>("/v1/mgmt/boards/master", data);
        return response;
    } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
            return error.response?.data || { code: "500", message: error.message, data: null };
...
export async function updateBoard(brdId: string, data: Partial<BoardMaster>): Promise<ApiResponse<void>> {
    try {
        const response = await apiClient.put<void>(`/v1/mgmt/boards/master/${brdId}`, data);
        return response;
    } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
            return error.response?.data || { code: "500", message: error.message, data: null };
...
export async function deleteBoard(brdId: string): Promise<ApiResponse<void>> {
    try {
        const response = await apiClient.delete<void>(`/v1/mgmt/boards/master/${brdId}`);
        return response;
    } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
            return error.response?.data || { code: "500", message: error.message, data: null };
